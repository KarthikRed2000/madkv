...
Each server is modeled as a state machine with a custom write-ahead log (WAL) stored as a plain file under the backer directory. The in-memory state is a sorted `std::map<string,string>`. Every mutating operation (`PUT`, `SWAP`, `DELETE`) is serialized as a compact log entry and appended to the WAL file with `fdatasync` before applying to the in-memory map. Recovery replays the WAL from the beginning to reconstruct the map. This approach uses fast sequential I/O for durability, decouples the command log from any storage engine, and prepares for state machine replication in Project 3.
